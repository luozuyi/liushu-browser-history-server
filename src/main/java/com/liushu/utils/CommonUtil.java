...
public class CommonUtil {
	...
	public static boolean isEmpty(String s) {
		if (null == s || "".equals(s) || "".equals(s.trim()) || Constants.NULL_STRING.equalsIgnoreCase(s)) {
			return true;
		} else {
			return false;
		}
	...
	public static String getIpAddr(HttpServletRequest request){
		final String localIpAddress="0:0:0:0:0:0:0:1";
		...
		if(ipAddress == null || ipAddress.length() == 0 || unknown.equalsIgnoreCase(ipAddress)) {
			...
			if(localHost.equals(ipAddress) || localIpAddress.equals(ipAddress)){
				//根据网卡取本机配置的IP
	...
	public static Boolean isFirstGetExpress(String spilt,String type){
		if (StringUtils.isBlank(spilt)){
			return true;
		}
		Boolean flag=true;
		//将记录标志位用逗号切割成String数组
		String[] chars=spilt.split(",");
		for (String c:
				chars) {
